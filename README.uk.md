...
Платіжний запит підтримує `amount` і необов’язковий `merchant_id`. Інші параметри, зокрема реальний PayLink `id` та підтвердження підпису, повертають HTTP 501 до початку операції. Вони поза документованим обсягом поточного адаптера; деталі — у [контракті сумісності](docs/COMPATIBILITY.md).
